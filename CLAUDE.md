...
- Root (`/`) — current production version: warm amber-brown background (`#2c2416`), gold accent (`#FFA400`).
- `v-old/` — previous version with dark navy background (`#0a1628`). Contains `index.html` + `at-*` sub-pages only.
- `nt-*/` — New Testament sub-page directories (currently empty, reserved for future content).

## Architecture

Six self-contained HTML files, each with all CSS and JavaScript embedded inline (no external `.css` or `.js` files):

- `index.html` — Main page: interactive grid of all 66 canonical books. Each book cell opens a modal with theological commentary. Includes a PNG export via `html2canvas` (CDN).
...
- `index.html` section labels (`<a class="slabel slabel-link" font-weight:700>`) link to each sub-page.
...
- Book header: `.book-eyebrow`, `.book-title` (with `<span>` for gold accent), `.book-meta` / `.book-tag`, `.book-verse`.
- Tabs: `<div class="tabs">` with `.tab-btn` buttons; `showTab(this, bookId, tabKey)` switches panels.
...
The `show(id)` function populates the modal from `DB[id]` on cell click. Modal uses solid `background:#2c2416` (no `backdrop-filter`) to avoid color bleed.
...
- All sub-pages use `--gold: #FFA400` as the primary UI accent. `--men: #3a8c6e` is defined in `at-profetas-menores` variables but no UI rule uses it — all UI references use `--gold`.
- Category accent colors in `index.html` are all amber/orange family — no blues or greens.

### Color palette (current version)

```text
--navy:       #2c2416   (background)
--navy-mid:   #3a3020   (sidebar, toolbar)
--navy-light: #4a3e28   (hover states)
--gold:       #FFA400   (primary accent)
--sand:       #f0e6d0   (body text)
--sand-dim:   #c8b898   (secondary text)
```
...
Each book has 6 cards. The `at-pentateuco` file has its own CSS definition for this component (just before `.card { }` in its `<style>` block).
